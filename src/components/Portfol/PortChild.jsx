...
function PortChild({alignment = "flex-row",borderColor = "aqua",title,description,id,image,link}) {
  return (
    <Motion.div initial={{y:17,opacity:0}}  transition={{duration:.4}} whileInView={{y:0,opacity:1}} viewport={{ once: true, amount: .3 }} className={`w-full gap-6 py-8 ${alignment} border-[var(--border-color)] border-b-[1px] flex max-lg:flex-col items-center justify-center`} >
        ...
        <div className='lg:w-[50%] w-full flex justify-center items-center lg:block flex-col  '>
            ...
            <p className='md:w-[86%] my-5 text-xl max-lg:text-center max-lg:px-1 text-[var(--text-muted)]' >{description}</p>
